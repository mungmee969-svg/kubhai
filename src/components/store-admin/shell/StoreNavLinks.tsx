...
function Badge({ count }: { count: number }) {
  ...
  return <span className="ml-auto inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-accent px-1 text-[10px] font-semibold text-navy-950">{count > 99 ? "99+" : count}</span>;
}

export function StoreNavLinks({ onNavigate, storeSlug, badges, hiddenHrefs }: {
  onNavigate?: () => void;
...
export function StoreMobileNav({ badges }: { badges?: NavBadgeCounts }) {
  const pathname = usePathname();
  return (
    <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-line bg-white/95 px-2 pt-2 pb-[calc(.5rem+env(safe-area-inset-bottom))] backdrop-blur lg:hidden">
      <div className="mx-auto grid max-w-lg grid-cols-4 gap-1">
        {MOBILE_NAV.map((item) => {
          const active = isActive(pathname, item.href, "exact" in item && item.exact);
          const count = badgeForHref(item.href, badges);
          return <Link key={item.href} href={item.href} className={`relative min-w-0 truncate rounded-xl px-1 py-2 text-center text-[11px] sm:text-xs ${active ? "bg-navy-800 text-white" : "text-navy-800"}`}>{item.label}{count ? <span className="absolute right-1 top-1 h-1.5 w-1.5 rounded-full bg-accent" /> : null}</Link>;
        })}
      </div>
    </nav>
  );
